...
function wrap(t,e){
	...
	h=t.text(null).append("tspan")
	.attr("x",p)
	.attr("y",a)
	.attr("dy",l+"em");
	o=i.pop();)n.push(o),
	h.text(n.join(" ")),
	h.node().getComputedTextLength()>e&&(n.pop(),
	h.text(n.join(" ")),n=[o],h=t.append("tspan").attr("x",p).attr("y",a).attr("dy",++s*r+l+"em").text(o))}
function wrapChord(t,e){
	t.each(function(){
		if("Administratief personeel"===d3.select(this).text())
			var t=70;
		else 
			var t=e;
		for(var o,
		i=d3.select(this),
		n=i.text().split(/\s+/).reverse(),
		s=[],
		r=0,
		a=1.1,
		p=i.attr("y"),
		l=i.attr("x"),
		h=parseFloat(i.attr("dy")),
		c=i.text(null).append("tspan")
		.attr("x",l)
		.attr("y",p)
		.attr("dy",h+"em");
		o=n.pop();)s.push(o),c.text(s.join(" ")),
		c.node().getComputedTextLength()>t&&(s.pop(),c.text(s.join(" ")),
		s=[o],c=i.append("tspan")
		.attr("x",l)
		.attr("y",p)
		.attr("dy",++r*a+h+"em").text(o))})}
